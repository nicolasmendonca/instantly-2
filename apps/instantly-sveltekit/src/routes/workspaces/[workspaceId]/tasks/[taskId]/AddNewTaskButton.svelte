<script lang="ts">
	...
	import { WORKSPACE_CONFIG } from '$src/application/constants';
	import { goto } from '$app/navigation';
	import { paths } from '$src/routes/paths';
	import Spinner from '$src/components/Spinner.svelte';
	import { tasksStore } from '$src/application/stores/tasksStore';
	import { setIsEditing } from './EditableTaskTitle.svelte';
...
<div class="w-[calc(100%_-_2rem)] mx-4">
	<button
		on:click={handleAddNewTask}
		disabled={status === 'LOADING'}
		class="w-full mb-2 rounded-xl border-2 border-primary-500 py-2 px-4 hover:bg-neutral-50 hover:bg-opacity-10 transition-all disabled:cursor-not-allowed"
	>
		{#if status === 'LOADING'}
